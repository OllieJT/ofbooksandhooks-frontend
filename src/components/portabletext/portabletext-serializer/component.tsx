...
import type { SerializerBlock_Book, SerializerBlock_Gallery, SerializerBlock_Highlight, SerializerBlock_People, SerializerBlock_Products, SerializerBlock_Video, SerializerMark_LinkInternal, SerializerMark_LinkExternal, SerializerBlock_Cta, SerializerMark_Marker, SerializerBlock_Image } from "@lib/groq/models";
...
export const serializers = {
	types: {
		...
		products: ({ node }: SerializerBlock_Products) => <BlockProducts title={node.title} products={node.products} />,
		video: ({ node }: SerializerBlock_Video) => <BlockVideo href={node.href} hasControls={node.hasControls} isAutoplay={node.isAutoplay} isLoop={node.isLoop} isMuted={node.isMuted} />,
		cta: ({ node }: SerializerBlock_Cta) => <BlockCta title={node.title} size={node.size} url={node.url} label={node.label} />,
	},
	marks: {
		...
		linkExternal: ({ children, mark }: SerializerMark_LinkExternal) => <MarkLink url={mark.url}>{children}</MarkLink>,
